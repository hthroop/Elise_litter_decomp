...
```{r}
# Import data, the CSV file...
Nitrogen <- read.csv("Nitrogen_content.csv")
```

### Data manipulation

```{r}
Nitrogen2 <- ddply(Nitrogen, c("Litter_substrate", "Month", "microsite"), summarise,
...
```{r}
Nitrogen3 <- ddply(Nitrogen, c("Litter_substrate", "Month", "microsite"), summarise,
...
write.csv(Nitrogen3, file = "Nitrogen3.csv")
Nitrogen4 <- read.csv("Nitrogen3.csv")
```
...
filtered_data <- subset(Nitrogen4, microsite %in% names(microsites_colors))
...
p <- ggplot(filtered_data, aes(x = Month, y = mean_nitrogen, shape = Litter_substrate, group = interaction(microsite, Litter_substrate))) +
  ...
  scale_y_continuous(limits = c(30, 65), breaks = seq(30, 65, by = 5))  # Set Y-axis limits and breaks
```
